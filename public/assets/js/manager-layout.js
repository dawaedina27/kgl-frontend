...
(() => {
  ...
  function injectBackButton() {
    ...
    const fallback = `${resolveAppBasePath()}pages/manager/dashboard.html`;
    button.addEventListener("click", () => {
  ...
  async function renderManagerLayout() {
    ...
    const topbarHost = document.getElementById("managerTopbar");
    const sidebarHost = document.getElementById("managerSidebar");

    if (!topbarHost || !sidebarHost) return;

    const activeNav = document.body.dataset.activeNav || "dashboard";
    const pageTitle = document.body.dataset.pageTitle || "Manager Dashboard";
    const username = localStorage.getItem("username") || "Manager";
    const displayName = localStorage.getItem("name") || username || "Manager";
    const branch = localStorage.getItem("branch") || "Maganjo";
    const role = localStorage.getItem("role");
    const isManager = role === "Manager";
    const isSalesAgent = role === "sales-agent" || role === "SalesAgent";
    const roleLabel = isManager ? "Manager" : "Sales Agent";
    const imageSrc = localStorage.getItem("profileImage") || "../../assets/images/profile.jpg";

    if (isSalesAgent) {
      window.location.href = `${resolveAppBasePath()}pages/sales-agent/dashboard.html`;
      return;
    }
    ...
    const dropdown = document.getElementById("managerProfileDropdown");
    const menu = document.getElementById("managerDropdownMenu");
    const logoutLink = document.getElementById("managerLogoutLink");
  ...
  window.renderManagerLayout = renderManagerLayout;
  document.addEventListener("DOMContentLoaded", renderManagerLayout);
})();
