...
(() => {
  ...
  function resolveAppBasePath() {
    ...
    const marker = "/pages/sales-agent/";
  ...
  function injectBackButton() {
    ...
    const fallback = `${resolveAppBasePath()}pages/sales-agent/dashboard.html`;
    button.addEventListener("click", () => {
  ...
  async function renderAgentLayout() {
    if (!requireAgentSession()) return;
    await syncSessionUserFromDatabase();
    applySidebarCollapsedState();

    const topbarHost = document.getElementById("salesAgentTopbar");
    const sidebarHost = document.getElementById("salesAgentSidebar");

    if (!topbarHost || !sidebarHost) return;

    const activeNav = document.body.dataset.activeNav || "dashboard";
    const pageTitle = document.body.dataset.pageTitle || "Sales Agent Dashboard";
    const username = localStorage.getItem("username") || "Sales Agent";
    const displayName = localStorage.getItem("name") || username || "Sales Agent";
    const branch = localStorage.getItem("branch") || "Maganjo";
    const imageSrc = localStorage.getItem("profileImage") || "../../assets/images/profile.jpg";

    topbarHost.innerHTML = `
      <header class="topbar">
        <div class="topbar-left">
          <img src="../../assets/images/logo1.png" class="logo-img" alt="Karibu Groceries LTD Logo">
          <button id="sidebarToggleBtn" class="sidebar-toggle-btn" type="button" aria-label="Toggle sidebar"></button>
        </div>
        <h2 class="topbar-left-h2">${escapeHtml(pageTitle)} | ${escapeHtml(branch)} Branch</h2>
        <div class="profile-dropdown" id="agentProfileDropdown">
          <img src="${escapeHtml(imageSrc)}" class="profile-img" alt="Profile">
          <div class="dropdown-menu" id="agentDropdownMenu">
            <a href="#" id="agentProfileLink"><i class="fa-solid fa-user"></i> Profile</a>
            <a href="#"><i class="fa-solid fa-gear"></i> Settings</a>
            <hr>
            <a href="#" id="agentLogoutLink">
      ...
      <aside class="sidebar">
        <div class="sidebar-scroll">
          <ul>
            ...
            <li data-nav="collections">
              <a href="collections.html">
                <i class="fa-solid fa-hourglass-half"></i>
                Credit Tracking
              </a>
            </li>
            <li data-nav="goods-condition">
              <a href="goods-condition.html">
        ...
        <div class="sidebar-footer">
          ...
          <div class="sidebar-user-meta">
            ...
            <p class="sidebar-user-role">Sales Agent</p>
          </div>
        </div>
      </aside>
    `;
    ...
    if (profileLink) {
      profileLink.addEventListener("click", (event) => {
        ...
        if (currentFile.toLowerCase() === "profile.html") {
          const fallback = sessionStorage.getItem("agent_profile_return") || "dashboard.html";
          sessionStorage.removeItem("agent_profile_return");
          window.location.href = fallback;
          return;
        }
        sessionStorage.setItem("agent_profile_return", currentFile || "dashboard.html");
  ...
  window.renderAgentLayout = renderAgentLayout;
  document.addEventListener("DOMContentLoaded", renderAgentLayout);
})();
